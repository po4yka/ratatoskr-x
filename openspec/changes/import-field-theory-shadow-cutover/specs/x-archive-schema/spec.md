...
operations, bookmark write audit events, legacy import runs, legacy import items, legacy shadow
reports, legacy transition approvals, tombstones, social sources, social source revisions,
...
Legacy import state SHALL bind source and parser versions, safe source evidence, deterministic
counts, normalized identity resolution, shadow input digests, and owner approval to one account
without credential, post-body, mutable-handle, or raw-URL evidence columns.
...
#### Scenario: Legacy transition evidence is account-scoped and secret-free
- **WHEN** the applied legacy import, item, shadow, and approval tables are inspected
- **THEN** they retain source and report digests, parser and source versions, identity outcomes, deterministic counts, and owner bindings without credential, post-body, mutable-handle, raw-URL, or source-path columns
